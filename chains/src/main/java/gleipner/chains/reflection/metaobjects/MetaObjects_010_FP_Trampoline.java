...
public class MetaObjects_010_FP_Trampoline extends GleipnerObject {

    public SinkGadget sinkGadget;
    public String taint;

    public MetaObjects_010_FP_Trampoline() {
    }

    public MetaObjects_010_FP_Trampoline(SinkGadget sinkGadget, String taint) {
    ...
    public int hashCode() {
        ...
        if (annotations1.length == annotations2.length && annotations2.length == annotations3.length) {
            for (Annotation a : annotations1) {
                for (Annotation aa : annotations3) {
